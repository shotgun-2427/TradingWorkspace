...
def EqualWeightOptimizer() -> Callable[[Dict[str, LazyFrame], Dict], LazyFrame]:
    """
    Averaging optimizer: equally weights model insights.
    Inputs:
      model_insights: { model_name: LazyFrame(["date", ...tickers...]) }
      backtest_results: ignored here (hook for future optimizers)
    Output (RAW, wide):
      LazyFrame(["date", ...tickers...]) with per-ticker weights (no clamping/padding/L1 scaling).
    """
    ...
    def run(model_insights: Dict[str, LazyFrame], backtest_results: Dict) -> LazyFrame:
        ...
        for lf in model_insights.values():
            ...
            long = (
                lf.unpivot(index="date", variable_name="ticker", value_name="w")
                .with_columns((pl.col("w") * (1.0 / n)).alias("w"))
            )
            longs.append(long)

        # concat/group_by stays lazy; pivot requires an eager DataFrame in your version
