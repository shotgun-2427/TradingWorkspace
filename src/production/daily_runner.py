...
def step_submit_orders(
    ...
    try:
        ...
        result = submit_paper_orders(profile=profile)
        if result.get("ok"):
            log.info("✓ Orders submitted. %s", result.get("message", ""))
        else:
            log.error("✗ Submit failed: %s", result.get("error"))
        return result
    except Exception as exc:
...
def main() -> None:
    ...
    parser.add_argument("--port", type=int, default=DEFAULT_PORT_PAPER)
