...
def _resolve_legacy_callable() -> Callable[..., Any]:
    mod = importlib.import_module("src.production.pipeline.append_ibkr_daily")

    candidate_names = [
        "append_ibkr_daily",
        "append_ibkr_daily_to_master",
        "run",
        "main",
    ]
    ...
    raise AttributeError(
        "Could not find a callable in src.production.pipeline.append_ibkr_daily. "
...
def append_ibkr_daily(profile: str = "paper", **kwargs: Any) -> dict[str, Any]:
    ...
    if isinstance(result, dict):
        ...
        result.setdefault("action", "append_ibkr_daily")
        return result
...
def run(profile: str = "paper", **kwargs: Any) -> dict[str, Any]:
    return append_ibkr_daily(profile=profile, **kwargs)
