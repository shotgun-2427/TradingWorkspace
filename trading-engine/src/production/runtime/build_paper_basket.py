...
def load_latest_targets(targets_path: Path) -> pd.DataFrame:
    ...
    # The targets file generated by ``generate_targets`` calls the date column
    # ``rebalance_date`` and the price column ``reference_price``. Older
    # versions used ``date`` / ``close``. Accept either.
    if "date" not in targets.columns and "rebalance_date" in targets.columns:
        targets["date"] = targets["rebalance_date"]
    if "close" not in targets.columns and "reference_price" in targets.columns:
        targets["close"] = targets["reference_price"]
    if "date" not in targets.columns:
        raise ValueError(
            "Targets parquet must contain a 'date' or 'rebalance_date' column "
            f"(found: {list(targets.columns)})"
        )
    ...
    if "close" not in targets.columns:
        # Last-resort fallback: look up close from the master price file at the
        # latest rebalance date so basket building doesn't blow up.
        try:
            prices_path = (
                targets_path.parent.parent / "prices" / "etf_prices_master.parquet"
            )
            if prices_path.exists():
                ...
                targets["symbol_upper"] = targets["symbol"].astype(str).str.upper()
                targets = targets.merge(
                    latest_per_sym, left_on="symbol_upper", right_on="symbol",
                    suffixes=("", "_master"), how="left",
                )
                targets = targets.drop(columns=["symbol_upper"])
                if "close" not in targets.columns and "close_master" in targets.columns:
                    targets["close"] = targets["close_master"]
        except Exception:
            pass
    if "close" not in targets.columns:
        raise ValueError(
            "Targets parquet must contain a 'close' or 'reference_price' "
            f"column (found: {list(targets.columns)})"
        )
